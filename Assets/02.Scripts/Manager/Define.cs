using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static class Define
{
    public const string bgmRoot = "BGM";
    public const string soundRoot = "Sound";

    // Ch1
    public const string ch1ClothSweepVfx = "ch1_cloth_sweep";
    public const string ch1ClothFootstepVfx = "ch1_cloth_footstep";

    // Ch2
    public const string ch2RunningFootVfx = "ch2_running_foot";
    public const string ch2ElevatorOpenVfx = "ch2_elevator_open";
    public const string ch2ElevatorCloseVfx = "ch2_elevator_close";

    // Ch3
    public const string ch3PhoneRingingVfx = "ch3_phone_ringing";
    public const string ch3PhoneRinging2Vfx = "ch3_phone_ringing2";
    public const string ch3EndCallVfx = "ch3_end_call";

    // Ch4
    public const string ch4PutObjectVfx = "ch4_put_object";

    // Ch5
    public const string ch5KnockingVfx = "ch5_knocking";
    public const string ch5SelectSeatVfx = "ch5_select_seat";
}
